...
async function initCroppie(imageUrl) {
  ...
  croppieDiv.style.width = '500px'
  croppieDiv.style.height = '500px'
  croppieContainerRef.value.appendChild(croppieDiv)

  // 创建新实例
  croppie = new Croppie(croppieDiv, {
    viewport: {width: 270, height: 450},
...
<template>
  <fieldset class="fieldset mt-0">
    ...
    <!-- 外层容器，用于触发 hover -->
    <div class="relative w-15 h-25 group">
      <!-- 图片/占位符 -->
      <div class="w-full h-full ring ring-primary ring-offset-2 ring-offset-base-100 rounded-box overflow-hidden">
        <img
          v-if="myBackgroundImage" :src="myBackgroundImage" alt="" class="object-cover w-full h-full">
        <div v-else class="text-6 w-full h-full rounded-box bg-base-300 flex justify-center font-bold items-center select-none text-center p-2">
          点击选择背景图片
        </div>
      </div>
      <div
        @click="fileInputRef.click()"
        class="absolute inset-0 w-full h-full rounded-box bg-black/50 flex flex-col justify-center items-center cursor-pointer backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300"
      >
  ...
  <dialog ref="modal-ref" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box p-6 bg-base-100 rounded-2xl shadow-2xl max-w-2xl transition-none">
